...
const AddTask = ({taskList, setTaskList}) => {
    ...
    const handleAdd = e => {
        ...
        if (projectName.trim() === '' || taskDescription.trim() === '') {
            setErrorMessage('Project name and task description cannot be empty');
            return;
        }
  ...
  return (
    <>
    <button className='bg-blue-600 text-white uppercase text-sm font-semibold
    py-1 mx-1.5 pl-2 pr-2 rounded hover: opacity-70' type='button'
        ...
        justify-center overflow-x-hidden overflow-y-auto fixed inset-0 z-100'>
            ...
            <form className='px-6 pt-6 pb-4'>
                ...
                focus:outline-none focus:bg-white'
                   ...
                   required
                />
                {errorMessage && <p className="text-red-500 text-center mt-2 mb-5">{errorMessage}</p>}
                </div>
                <div>
                    <label className='track-wide uppercase text-gray-700 text-xs
                font-semibold mb-2 block'
            ...
            <div className='flex justify-end p-6 border-t border-slate-200 rounded-b'>
                ...
                uppercase text-sm px-6 py-3 rounded hover:opacity-70' onClick={handleAdd}>Add Task</button>
            </div>
            </div>
        </div>
